...
const collider_radius : float = 0.5
const collider_height : float = 2.0
func add_collider(color : Color):
	...
	mesh.mesh.radius = collider_radius
	mesh.mesh.height = collider_height
...
func Flying_Test(delta : float) -> void:
	...
	Fly_Force_UP = Vector3(0,10,0)
	Fly_Force_DOWN = Vector3(0,-10,0)
...
func Calculate_Shot():
	...
	if Input.is_action_pressed("chara1_shots"):
		var b = bullet.new()
...
func Calculate_Illuminate():
	...
	#else:print("El fuckiing nodo no quiere funcionar")
